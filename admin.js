...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ==========================================================
       Dữ liệu giả lập
       ========================================================== */
    let files = Array.from({ length: 0 }, (_, i) => ({ name: `tệp_${i + 1}.txt`, date: new Date().toLocaleDateString('vi-VN'), status: 'Đã duyệt' }));
    let users = Array.from({ length: 0 }, (_, i) => ({ username: `user${i + 1}`, role: 'user' }));
    ...
    const updateFileTable = (data = files) => {
        ...
        tbody.innerHTML = '';
        if (data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center;">Chưa có tệp nào.</td></tr>';
            return;
        }
        data.forEach(file => {
    ...
    const updateHistoryTable = (data = history) => {
        ...
        tbody.innerHTML = '';
        if (data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align: center;">Chưa có lịch sử nào.</td></tr>';
            return;
        }
        data.forEach(entry => {
            const row = document.createElement('tr');
    ...
    window.addFile = () => {
        ...
        if (name && name.trim() !== '') {
            files.unshift({ name, date: new Date().toLocaleDateString('vi-VN'), status: 'Chưa duyệt' });
            updateFileTable();
            updateDashboardData();
        } else if (name !== null) {
    ...
    window.changeRole = (username) => {
        ...
        if (newRole && (newRole.trim().toLowerCase() === 'admin' || newRole.trim().toLowerCase() === 'user')) {
            ...
            showModal('Lỗi', 'Quyền không hợp lệ. Vui lòng nhập "admin" hoặc "user".');
        }
    };
    ...
    window.addHistory = () => {
        const name = prompt('Nhập tên tệp:');
        if (name && name.trim() !== '') {
            history.unshift({ name, date: new Date().toLocaleDateString('vi-VN'), result: 'Tóm tắt thành công' });
            updateHistoryTable();
            updateDashboardData();
        } else if (name !== null) {
            showModal('Lỗi', 'Tên tệp không được để trống.');
    ...
    window.searchHistory = () => {
        ...
        const filteredHistory = history.filter(entry => entry.name.toLowerCase().includes(searchTerm));
